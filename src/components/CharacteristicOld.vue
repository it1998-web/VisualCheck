<template>
  <div class="Charac_teristic">
    ...
    <!-- 引车员信息 -->
      ...
      <!-- <van-field style="color:#323233;" :readonly="true" v-model="F1Result.wgjcjyysfzh" label="身份证号" placeholder="外检员身份证号" /> -->
      <!-- 总判定 -->
       <van-field style="color:#E21918;" :readonly="true" v-model="qualified" label="总判定" placeholder="" />
       </div>

  <van-cell-group  title="唯一性检查" style="border-bottom: 15px solid #f5f5f5;">
     <van-cell  center title="号牌号码和分类" v-show="F1Result.rhplx==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rhplx)" @change="changeSwitch(F1Result.rhplx,'rhplx')" :disabled="F1Result.rhplx==0?true:false"  active-color="#34C759" v-model="F1Result.rhplx==1?true:false" size="24" />
       </template>
    </van-cell>
    <van-cell  center title="车辆品牌和型号" v-show="F1Result.rppxh==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rppxh)" @change="changeSwitch(F1Result.rppxh,'rppxh')" :disabled="F1Result.rppxh==0?true:false"  active-color="#34C759" v-model="F1Result.rppxh==1?true:false" size="24" />
       </template>
    </van-cell>
     <van-cell  center title="车辆识别代号" v-show="F1Result.rvin==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rvin)" @change="changeSwitch(F1Result.rvin,'rvin')" :disabled="F1Result.rvin==0?true:false"  active-color="#34C759" v-model="F1Result.rvin==1?true:false" size="24" />
       </template>
    </van-cell>
     <van-cell  center title="发动机号码/驱动电机号码" v-show="F1Result.rfdjh==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rfdjh)" @change="changeSwitch(F1Result.rfdjh,'rfdjh')" :disabled="F1Result.rfdjh==0?true:false"  active-color="#34C759" v-model="F1Result.rfdjh==1?true:false" size="24" />
       </template>
    </van-cell>
     <van-cell  center title="车身颜色和车辆外形" v-show="F1Result.rcsys==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rcsys)" @change="changeSwitch(F1Result.rcsys,'rcsys')" :disabled="F1Result.rcsys==0?true:false"  active-color="#34C759" v-model="F1Result.rcsys==1?true:false" size="24" />
       </template>
    </van-cell>
  </van-cell-group>
   <van-cell-group  title="特征参数" style="border-bottom: 15px solid #f5f5f5;">
      ...
      <van-cell  center title="整备质量" v-show="F1Result.rzbzl==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rzbzl)" @change="changeSwitch(F1Result.rzbzl,'rzbzl')" :disabled="F1Result.rzbzl==0?true:false"  active-color="#34C759" v-model="F1Result.rzbzl==1?true:false" size="24" />
       </template>
     </van-cell>
      ...
      <van-cell  center title="核定载质量" v-show="F1Result.rhdzll==0?false:true">
       <template #right-icon>
         <van-switch :inactive-color="swictcColor(F1Result.rhdzll)" @change="changeSwitch(F1Result.rhdzll,'rhdzll')" :disabled="F1Result.rhdzll==0?true:false"  active-color="#34C759" v-model="F1Result.rhdzll==1||0?true:false" size="24" />
       </template>
     </van-cell>
     <van-cell  center title="栏板高度" v-show="F1Result.rlbgd==0?false:true">
      ...
      <van-cell  center title="后轴钢板弹簧篇数" v-show="F1Result.rhzgbthps==0?false:true">
       <template #right-icon>
   ...
   <!-- 安全装置 -->
   <van-cell-group title="安全装置" style="border-bottom: 15px solid #f5f5f5;">
  ...
  data() {
   ...
   mounted () {
    ...
    //  this.$store.dispatch('getState',this.F1Result);
     ...
     this.$store.dispatch('getqualified','1');//总检验结果是否合格
   },
   methods: {
